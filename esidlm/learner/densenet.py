...
class EntityDenseNetLearner(BaseLearner):
    ...
    def run_model_training(self):
        ...
        x_cont_train, x_cate_train = self._preprocess_input_data(
            input_data=train_data,
            is_train=True
        )
        x_cont_valid, x_cate_valid = self._preprocess_input_data(
            input_data=valid_data,
            is_train=False
        )

        train_set = EntityDenseNetDataset(x_cont_train, x_cate_train, 
                                          train_data[self.data_config["target_col"]])
        valid_set = EntityDenseNetDataset(x_cont_valid, x_cate_valid,
        ...
        x_cont_test, x_cate_test = self._preprocess_input_data(
            input_data=test_data,
            is_train=False
        )
        test_set = EntityDenseNetDataset(x_cont_test, x_cate_test, 
    ...
    def run_model_inference(self):
        ...
        model = LitEntityDenseNetModel.load_from_checkpoint(self.model_config["model_checkpoint_path"])
